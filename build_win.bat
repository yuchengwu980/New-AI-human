...
if errorlevel 1 (
  ...
  echo Try: .venv\Scripts\python -m pip install pyinstaller -i https://pypi.tuna.tsinghua.edu.cn/simple
  exit /b 1
)
...
.venv\Scripts\pyinstaller --noconfirm --clean --windowed --name %APP_NAME% --paths . --hidden-import app --hidden-import app.desktop.main_desktop --hidden-import pydantic --hidden-import pydantic_core --hidden-import pydantic_settings --collect-submodules app --collect-all pydantic_settings --add-data "scripts;scripts" app\desktop\main_desktop.py
if errorlevel 1 (
