...
桌面窗口说明（全部在一个应用窗口完成）：
- 评论输入适配层（阶段B）：输入方式选择 `manual_ui` / `http_post_/comment` / `file_tail`，并显示当前状态
...
- OBS 输出器（阶段A）：可配置并连接 OBS WebSocket，自动把“最新回复文本”推送到字幕源，并触发媒体源播放“最新 wav”
...
1. 在 OBS 创建两个 Source（名称要和桌面端一致）：
   - Text/GDI+：`字幕`
   - Media Source：`TTS音频`
2. 在 OBS 启用 obs-websocket（默认端口 `4455`，设置密码）。
3. 在桌面客户端“OBS 输出器（阶段A）”中填入 Host/Port/Password、源名称，勾选启用后点“连接 OBS”。
4. 点“测试推送”验证。
5. 输入“你好”并发送：应看到 OBS 字幕变化并听到媒体源播放最新 wav。

> 该联动是对现有输出文件的后置读取与推送，不改变 `output/audio` 与 `output/events` 的原有写法。


### 2.4.2 Stage C：LLM/TTS Provider（并列新增，失败回退）

> 默认仍是离线可用：`LLM_PROVIDER=mock` + `TTS_PROVIDER=silent`。

1) 在 `.env` 配置（可选联网）：

```env
LLM_PROVIDER=openai
LLM_OPENAI_API_KEY=你的key
LLM_OPENAI_MODEL=gpt-4o-mini
LLM_OPENAI_TIMEOUT_SECONDS=10

TTS_PROVIDER=openai
TTS_OPENAI_API_KEY=你的key
TTS_OPENAI_MODEL=gpt-4o-mini-tts
TTS_OPENAI_VOICE=alloy
TTS_OPENAI_TIMEOUT_SECONDS=10
```

2) 桌面端 `Provider 选择（阶段C）` 区域可实时选择：
- LLM Provider：`mock/openai`
- TTS Provider：`silent/openai`
- 点击“应用 Provider 配置”生效。

3) 状态观察：
- UI 会显示 provider 状态（耗时、是否 fallback、错误原因）。
- 任一在线 provider 失败会自动回退：
  - LLM 回退到 mock 模板回复
  - TTS 回退到静音 wav
- 输出目录结构保持不变：`output/audio`、`output/events`。

4) 从 0 到可播出（3分钟）：
- 启动桌面端：`run_desktop.bat`
- 若有 key：把 LLM/TTS 切到 `openai` 并应用；无 key 可保持默认 mock/silent。
- 输入“你好”发送：
  - 生成 `output/events/last_event.json`
  - 生成 `output/audio/*.wav`
  - 若已配置 OBS（阶段A），字幕会更新并触发音频播放。
...
6. 对 `dist/NewAIHuman/NewAIHuman.exe --selfcheck` 做 smoke test
7. 生成 `NewAIHuman_Win64.zip`（包含 exe、output 目录和 `README_使用说明.txt`）
...
- workflow 内含 exe 自检步骤（`--selfcheck`）确保产物可启动
...
#### HTTP 方式：`POST /comment`

```bash
curl -X POST http://127.0.0.1:8000/comment \
  -H "Content-Type: application/json" \
  -d '{"user_id":"u100","text":"你好","ts":1730000000,"msg_id":"m-001"}'
```
...
#### 文件追加方式（Desktop file tail）

在桌面端把输入方式切到 `file_tail`，点击“启动文件监听”，默认监听：
- `output/input_comments.txt`

每追加一行会自动喂给现有处理链。支持两种格式：

1) JSON 行：
```text
{"user_id":"u2","text":"问下价格","ts":1730000001,"msg_id":"m-002"}
```
...
> 阶段B仅在入口新增适配层，去重/限频/状态机/回复生成与输出写法保持原逻辑。
